# -*- coding: utf-8 -*-
"""
Created on Tue Jul  6 21:47:43 2021
...
b11=np.empty((108,37,37,144))  #对Va求距平分量
for t in range(108):
    b11[t,:,:,:]=b[t,:,:,:]-b10
b20=np.nanmean(b11,axis=3)  #对Va距平分量求纬圈平均
ttv1=np.empty((108,37,37))  #求TTv1
ttv1=a20*b20
ttv10=np.nanmean(ttv1,axis=0)  #对ttv1求时间平均
ttv11=np.empty((37,37))  #求经向散度TTv11
...
plt.hlines(50,-45,60,linewidth=1,color='k',linestyle=':')     
plt.plot(ttv13[:15]*(1e+14),levs,linewidth=1,color='#EE82EE',label='TFv1',marker='.') 
n.invert_yaxis()          
n.set_yscale('symlog')
...
plt.xticks(np.arange(-1.8,0.9,0.2))         
plt.xlim(-1.8,0.8)
n.set_xticklabels([-1.8,-1.6,-1.4,-1.2,-1.0,-0.8,-0.6,-0.4,-0.2,0.0,0.2,0.4,0.6,0.8],fontfamily="Times New Roman",fontsize=9.5)
 
#%%分割cell        
""" 计算TTv2 """      
b21=np.empty((108,37,37,144))  #对Va距平分量求纬圈距平
for y in range(144):
    b21[:,:,:,y]=b11[:,:,:,y]-b20
ttv2=np.empty((108,37,37,144))  #求TTv2 
for y in range(144):
    ttv2[:,:,:,y]=b21[:,:,:,y]*a20  
ttv20=np.nanmean(ttv2,axis=0)    #对TTv2求时间平均
ttv21=np.empty((37,37,144))   #求纬向散度TTv20    
for x in range(1,36):
    ttv21[:,0,:]=(ttv20[:,1,:]-ttv20[:,0,:])/dy
    ttv21[:,x,:]=(ttv20[:,x+1,:]-ttv20[:,x-1,:])/(2*dy) 
    ttv21[:,36,:]=(ttv20[:,36,:]-ttv20[:,35,:])/dy

ttv22=ttv21[:,16:24,83:107]
...
plt.plot(ttv23[:15]*(1e+14),levs,linewidth=1,color='#FFA500',label='TFv2',marker='.') 
...
plt.plot(ttv33[:15]*(1e+14),levs,linewidth=1,color='#3CB371',label='TFv3',marker='.')         

#%%分割cell  
""" 计算TTv4 """  
ttv4=np.empty((108,37,37,144))  #求TTv4 
for y in range(144):
    ttv4[:,:,:,y]=a21[:,:,:,y]*b21[:,:,:,y]   
ttv40=np.nanmean(ttv4,axis=0)    #对TTv2求时间平均
ttv41=np.empty((37,37,144))   #求纬向散度TTv20        
for x in range(1,36):
    ttv41[:,0,:]=(ttv40[:,1,:]-ttv40[:,0,:])/dy
    ttv41[:,x,:]=(ttv40[:,x+1,:]-ttv40[:,x-1,:])/(2*dy) 
    ttv41[:,36,:]=(ttv40[:,36,:]-ttv40[:,35,:])/dy
ttv42=ttv41[:,16:24,83:107]
...
plt.plot(ttv43[:15]*(1e+14),levs,linewidth=1,color='#1E90FF',label='TFv4',marker='.') 

#%%分割cell 
""" 计算TTv总 """
ttv=ttv13+ttv23+ttv33+ttv43
np.savetxt('tfvls76.csv',ttv,fmt='%.40f',delimiter=',')
plt.plot(ttv[:15]*(1e+14),levs,linewidth=1,color='k',label='TFv-Total',marker='.')
