# -*- coding: utf-8 -*-
"""
Created on Tue Jul  6 10:34:51 2021
...
plt.hlines(50,-260,60,linewidth=1,color='k',linestyle=':')      
plt.plot([0]*15,levs,linewidth=1,color='#EE82EE',label='SFu1',marker='.')
n.invert_yaxis()          
n.set_yscale('symlog')
...
plt.xticks(np.arange(-100,40,10))     
plt.xlim(-100,30)
n.set_xticklabels(np.arange(-100,40,10),fontfamily="Times New Roman",fontsize=9.5)       
...
stu2=np.empty((37,37,144))  #求STu2 
for y in range(144):
    stu2[:,:,y]=a22[:,:,y]*b11[:,:]    
stu20=np.empty((37,37,144))   #求纬向散度STu20    
...
plt.plot(-stu22[:15]*(1e+14),levs,linewidth=1,color='#FFA500',label='SFu2',marker='.') 
...
stu3=np.empty((37,37,144))  #求STu3
for y in range(144):
    stu3[:,:,y]=b22[:,:,y]*a11[:,:]   
stu30=np.empty((37,37,144))  #求纬向散度STu30     
for x in range(37):  
    dx=6378388.0*np.cos(lats[x]*(np.pi/180.0))*dlon
    for y in range(1,143):
...
plt.plot(-stu32[:15]*(1e+14),levs,linewidth=1,color='#3CB371',label='SFu3',marker='.')        
# print(stu35[15]) 

#%%分割cell 
""" 计算STu4 """    
stu4=np.empty((37,37,144))  #求STu3       
for y in range(144):
    stu4[:,:,y]=a22[:,:,y]*b22[:,:,y]  
stu40=np.empty((37,37,144))   #求纬向散度STu40 
for x in range(37):  
    dx=6378388.0*np.cos(lats[x]*(np.pi/180.0))*dlon
    for y in range(1,143):
...
plt.plot(-stu42[:15]*(1e+14),levs,linewidth=1,color='#1E90FF',label='SFu4',marker='.')  
#%%分割cell 
""" 计算STu总 """
stu=stu22+stu32+stu42
np.savetxt('sfuls76.csv',stu,fmt='%.40f',delimiter=',')
plt.plot(-stu[:15]*(1e+14),levs,linewidth=1,color='k',label='SFu-Total',marker='.')
