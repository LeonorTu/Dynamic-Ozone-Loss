# -*- coding: utf-8 -*-
"""
Created on Sun Jul  4 20:57:24 2021
...
b11=np.empty((108,19,37,144))  #对Ua求距平分量
for t in range(108):
    b11[t,:,:,:]=b[t,:,:,:]-b10
b20=np.nanmean(b11,axis=3)  #对Ua距平分量求纬圈平均
ttu1=np.empty((108,19,37))  #求TTu1
ttu1=a20*b20   
...
plt.hlines(50,-7,3,linewidth=1,color='k',linestyle=':')     
plt.plot([0]*10,levs/100.0,linewidth=1,color='#EE82EE',label='TFu1',marker='.') 
n.invert_yaxis()          
n.set_yscale('symlog')
...
plt.xticks(np.arange(-3.5,1.6,0.5))         
plt.xlim(-3.5,1.7)
n.set_xticklabels(np.arange(-3.5,1.6,0.5),fontfamily="Times New Roman",fontsize=9.5) 
      
""" 计算TTu2 """      
b21=np.empty((108,19,37,144))  #对Ua距平分量求纬圈距平
for y in range(144):
    b21[:,:,:,y]=b11[:,:,:,y]-b20
ttu2=np.empty((108,19,37,144))  #求TTu2 
for y in range(144):
    ttu2[:,:,:,y]=b21[:,:,:,y]*a20  
ttu20=np.nanmean(ttu2,axis=0)    #对TTu2求时间平均
ttu21=np.empty((19,37,144))   #求纬向散度TTu20 
...
plt.plot(-ttu23[9:]*(1e+14),levs/100.0,linewidth=1,color='#FFA500',label='TFu2',marker='.')

""" 计算TTu3 """
a21=np.empty((108,19,37,144))  #对O3距平分量求纬圈距平
for y in range(144):
    a21[:,:,:,y]=a11[:,:,:,y]-a20
ttu3=np.empty((108,19,37,144))  #求TTu3 
for y in range(144):
    ttu3[:,:,:,y]=a21[:,:,:,y]*b20  
ttu30=np.nanmean(ttu3,axis=0)    #对TTu2求时间平均
ttu31=np.empty((19,37,144))   #求纬向散度TTu20    
for x in range(37):  
    dx=6378388.0*np.cos(lats[x]*(np.pi/180.0))*dlon
    for y in range(1,143):
...
plt.plot(-ttu33[9:]*(1e+14),levs/100.0,linewidth=1,color='#3CB371',label='TFu3',marker='.')         

""" 计算TTu4 """  
ttu4=np.empty((108,19,37,144))  #求TTu4 
for y in range(144):
    ttu4[:,:,:,y]=a21[:,:,:,y]*b21[:,:,:,y]   
ttu40=np.nanmean(ttu4,axis=0)    #对TTu2求时间平均
ttu41=np.empty((19,37,144))   #求纬向散度TTu20    
for x in range(37):  
    dx=6378388.0*np.cos(lats[x]*(np.pi/180.0))*dlon
    for y in range(1,143):
...
plt.plot(-ttu43[9:]*(1e+14),levs/100.0,linewidth=1,color='#1E90FF',label='TFu4',marker='.') 

""" 计算TTu总 """
ttu=ttu23+ttu33+ttu43
np.savetxt('tfuls74.csv',ttu,fmt='%.40f',delimiter=',')
plt.plot(-ttu[9:]*(1e+14),levs/100.0,linewidth=1,color='k',label='TFu-Total',marker='.')
